...
#vector_store.add(ray_docs_nodes)
...
class EmbedNodes:
    ...
    def __call__(self, node_batch: Dict[str, List[Node]]) -> Dict[str, List[Node]]:
        ...
        for node, text in zip(nodes, text):
            node.text = text
...
all_docs_gen = Path("/mnt/shared_storage/docs.ray.io/").rglob("*")
...
#TODO add the metadata and the node IDs
ray_docs_nodes = []
...
# Step 6: Store the embedded nodes in Pinecone.
print("Storing Ray Documentation embeddings in vector index.")
from llama_index.schema import TextNode
from llama_index.vector_stores.types import NodeWithEmbedding

def convert_text_nodes_to_node_with_embeddings(text_nodes: List[TextNode]) -> List[NodeWithEmbedding]:
    return [NodeWithEmbedding(node=text_node, embedding=text_node.embedding) for text_node in text_nodes]

vector_store.add(convert_text_nodes_to_node_with_embeddings(ray_docs_nodes))
...
all_blogs_gen = Path("/mnt/shared_storage/www.anyscale.com/blog/").rglob("*")
all_blogs = [{"path": blog.resolve()} for blog in all_blogs_gen]

ds = ray.data.from_items(all_blogs)
loaded_blogs = ds.flat_map(load_and_parse_files)
nodes = loaded_blogs.flat_map(convert_documents_into_nodes)
embedded_nodes = nodes.map_batches(
    EmbedNodes, 
    batch_size=100, 
    compute=ActorPoolStrategy(size=1), 
...
vector_store.add(convert_text_nodes_to_node_with_embeddings(ray_blogs_nodes))
